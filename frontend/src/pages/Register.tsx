...
import { User as UserIcon, Mail, Lock, Shield, ArrowRight, Zap, AlertCircle } from 'lucide-react';
      ...
      <div className="w-full max-w-md animate-fade-in relative">
        <div className="glass rounded-3xl p-8 shadow-2xl border-white/20">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground mt-2">Join GigFlow and start managing leads</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-background/50 border border-border rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-medium"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-bold ml-1 text-foreground/80">Select Role</label>
              <div className="relative group">
                <Shield className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-primary transition-colors" size={18} />
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 bg-background/50 border border-border rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-medium appearance-none"
                >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-primary text-white py-3.5 rounded-xl font-bold shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:hover:translate-y-0 group mt-4"
            >
              {loading ? (
